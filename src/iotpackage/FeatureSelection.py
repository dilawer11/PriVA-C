...
class FeatureSelector():
    ...
    def __init__(self, vals, save_path, pb=True):
        
        self.n_all_ = int(vals['n_all'])
        self.n_tcp_ = int(vals['n_tcp'])
        self.n_udp_ = int(vals['n_udp'])
        self.n_proto_ = int(vals['n_proto'])
        self.one_hot_encode_ = bool(vals['one_hot_encode'])
        self.simple_groups = vals['simple_groups']
        self.dict_groups = vals['dict_groups']
        self.simple_features = vals['simple_features']
        self.dict_features = vals['dict_features']

        l.info(f"Feature Selector Initialized")
        l.info(f"simple_groups: {self.simple_groups}, dict_groups: {self.dict_groups}")
        l.info(f"simple_features: {self.simple_features}, dict_features: {self.dict_features}")
        l.info(f"n_all: {self.n_all_}, n_tcp: {self.n_tcp_}, n_udp: {self.n_udp_}, n_proto: {self.n_proto_}, one_hot_encode: {self.one_hot_encode_}")
    ...
    def save(self):
        path = os.path.join(self.save_path, self.fs_fn)
