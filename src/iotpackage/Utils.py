...
from iotpackage.__vars import simpleFeatureGroups, dictFeatureGroups
import pathlib
import pickle
...
def strRound2(val, dp=2):
    st = '{:.dpf}'.replace('dp', str(dp))
    return st.format(val)

def sumFeatureStatsByGroup(fi):
    if not 'feature' in fi.columns:
        raise AttributeError(f"No 'feature' column in fi: {fi.columns}")
    fi['feature_group'] = fi['feature'].apply(getFeatureGroup)
    return fi.groupby('feature_group').sum().drop(columns=['feature']).reset_index()

def featureImporationByGroup(fi):
    featureStats = sumFeatureStatsByGroup(fi)
    return featureStats.sort_values(by='importance', ascending=False).reset_index(drop=True)

def createFeatureList(features):
    feature_dict = {}
    from iotpackage.Utils import getFeatureGroup
    for feature in features:
        feature_group = getFeatureGroup(feature)
        if feature_group is None: continue
        if feature_group not in feature_dict: feature_dict[feature_group] = [feature]
        else: feature_dict[feature_group].append(feature)
    return list(feature_dict.items())
...
def getPDPathFromIRPath(ir_path, ir_base_path, pd_base_path):
    if not os.path.isdir(ir_base_path): raise FileNotFoundError(f"ir_base_path={ir_base_path} not found")
    if not os.path.isdir(pd_base_path): raise FileNotFoundError(f"pd_base_path={pd_base_path} not found")
